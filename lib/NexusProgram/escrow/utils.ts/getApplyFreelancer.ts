...
export const getApplyFreelancer = async (
    anchorWallet: any,
    ...
    const [reviever] = web3.PublicKey.findProgramAddressSync(
        [
    ...
    const ESCROW_OFFSET = 73; // it 57 OR 58 since we are dealing with Option
    const programAccounts = await connection.getProgramAccounts(
        NEXUS_ADDRESS,
        {
            filters: [{ memcmp: { offset: ESCROW_OFFSET, bytes: reviever.toBase58() } }],
            commitment,
        }
    );
